...
class AStar(object):
    ...
    def get_neighbors(self, x):
        # TODO: fill me in!
        #checking 8 neighbours clockwise starting from the north
        x_change = np.array([0., 1., 1., 1., 0., -1., -1., -1.])*self.resolution
        y_change = np.array([1., 1., 0., -1., -1., -1., 0., 1.])*self.resolution
        neigh = [] #list of neighbors

        for i in range(8):
            current_neighbour = self.snap_to_grid((x[0] + x_change[i], x[1] + y_change[i]))
            if self.is_free(current_neighbour):
                neigh.append(current_neighbour)
    ...
    def find_best_f_score(self):
        ...
        #min is a builtin function here
    ...
    def solve(self):
        ...
        while len(self.open_set)>0: #till there are elements in the open set to be explored
            # TODO: fill me in!

            # getting the state in the open set with the lowest F score:
            x_current = self.find_best_f_score()

            #checking if the goal has been reached and reconstructing path along with returning true
            if x_current == self.x_goal:
                self.path = self.reconstruct_path()
                return True

            # remove x_current from the open set and add to the closed set:
            x_current_id = self.open_set.index(x_current)
            del self.open_set[x_current_id]
            self.closed_set.append(x_current)

            # checking in all free neighbors of x_current:
            for x_neigh in self.get_neighbors(x_current):
                if x_neigh in self.closed_set:
                    # x_neigh has already been visited. We should check different neighbor
                    continue
                ...
                #if neigh not in open set, add it there. and if it was there, ccompare its gscore with
                #the new prospect
                if x_neigh not in self.open_set:
                    self.open_set.append(x_neigh)
                elif tentative_g_score > self.g_score[x_neigh]:
                    # there already exist a better path from x_init to x_neigh, so continue with the next neighbor:
                    continue

                # (if we have found a new best path from x_init to x_neigh:)
                self.came_from[x_neigh] = x_current
                h_score = self.distance(x_neigh, self.x_goal)
                self.g_score[x_neigh] = tentative_g_score
                self.f_score[x_neigh] = tentative_g_score + h_score
